    ...
    def editCustomClicked(self, button):
        ...
        for xnatLevel, _button in self.editCustomButtons.iteritems():
            if button == _button:
                ...
                #--------------------
                self.MODULE.SettingsWindow.\
                    showSettingWidget(self.MODULE.Settings['METADATA'].title) 

                #--------------------
                # Expand the collapsible that had the same
                # XnatLevel as the button that called this function.
                #--------------------
                self.MODULE.Settings['METADATA'].\
                    MetadataEditorSets['main'].collapsibles[xnatLevel].\
                    setChecked(True)
            else:
                self.MODULE.Settings['METADATA'].\
                    MetadataEditorSets['main'].collapsibles[xnatLevel].\
