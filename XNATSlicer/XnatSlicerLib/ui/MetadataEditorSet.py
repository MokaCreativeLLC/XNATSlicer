...
class MetadataEditorSet(qt.QFrame):
    ...
    def constructManager(self):
        ...
        for xnatLevel in Xnat.path.DEFAULT_LEVELS:
            ...
            # add to layout.
            #
            self.defaultMetadataEditors[xnatLevel] = MetadataEditor_Default(xnatLevel)
            ...
            # add to layout.
            # 
            self.customMetadataEditors[xnatLevel] = MetadataEditor_Custom(xnatLevel)
    ...
    def editCustomClicked(self, button):
        ...
        for xnatLevel, _button in self.editCustomButtons.iteritems():
            if button == _button:
                ...
                #--------------------
                self.MODULE.SettingsWindow.showSettingWidget(self.MODULE.Settings['METADATA'].title) 

                #--------------------
                # Expand the collapsible that had the same
                # XnatLevel as the button that called this function.
                #--------------------
                self.MODULE.Settings['METADATA'].MetadataEditorSets['main'].collapsibles[xnatLevel].setChecked(True)
            else:
                self.MODULE.Settings['METADATA'].MetadataEditorSets['main'].collapsibles[xnatLevel].setChecked(False)
    ...
    def update(self):
        ...
        for key in self.customMetadataEditors:
            ...
            try:
                ...
                checkedItems = self.defaultMetadataEditors[key].checkedMetadataItems + \
                               self.customMetadataEditors[key].checkedMetadataItems
