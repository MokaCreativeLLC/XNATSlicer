...
class Settings(qt.QScrollArea):
    ...
    def __init__(self, SettingsFile, title = None):
        ...
        self.EVENT_TYPES = list(set(Settings.EVENT_TYPES + 
                                    self.EVENT_TYPES))
        ...
        self.syncMethods_ToFile = {}
        self.syncMethods_FileTo = {}
        self.DEFAULTS = {}
    ...
    def __insertSyncMethod(self, methodDict, section, method):
        """
        """
        if not section in methodDict:
            methodDict[section] = []
        methodDict[section].append(method)





    def addSyncMethod_ToFile(self, section, method):
        """
        """
        self.__insertSyncMethod(self.syncMethods_ToFile, section, method)
    ...
    def addSyncMethod_FileTo(self, section, method):
        """
        """
        self.__insertSyncMethod(self.syncMethods_FileTo, section, method)
    ...
    def syncToFile(self):
        """
        """
        MokaUtils.debug.lf('BEGIN SyncToFile!\n\n')
        if not self.currXnatHost:
            print "NO HOST, returning"
            return

        if hasattr(self, 'DEFAULTS'):
            MokaUtils.debug.lf('Begin Apply defaults!\n\n')
            self.applyDefaultsIfNeeded()


        for syncType, methods in self.syncMethods_ToFile.iteritems():
    ...
    def getStorageTag(self, *suffixArgs):
        ...
        for suffixArg in suffixArgs:
            returnStr += '-' + suffixArg.title()
        return MokaUtils.string.toCamelCase(returnStr).replace('settings', '')
