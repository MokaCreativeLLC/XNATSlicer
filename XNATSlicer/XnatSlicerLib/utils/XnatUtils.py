...
class XnatUtils(object):
    """
    XnatUtils contains a variety of utility methods
    for XNAT interactions used throughout the
    XNATSlicer suite. 

    All of its methods are static; the user doesn't 
    need to instantiate the XnatUtils class to
    use it.
    """
    ...
    def isRecognizedFileExt(ext):
        ...
        arr = (GLOB_DICOM_EXTENSIONS + 
               ...
               GLOB_SLICER_PACKAGE_EXTENSIONS)
        for item in arr:
            if ext == item:
                return True
    ...
    def isAnalyze(fileName = None):
        ...
        @rtype: boolean
        """
        return XnatUtils.isExtension(fileName, GLOB_ANALYZE_EXTENSIONS)
    ...
    def isMRML(fileName = None): 
        ...
        @rtype: boolean
        """    
        return XnatUtils.isExtension(fileName, GLOB_MRML_EXTENSIONS)
    ...
    def isScenePackage( ext = None):
        ...
        @rtype: boolean
        """
        return XnatUtils.isExtension(ext, GLOB_SLICER_PACKAGE_EXTENSIONS)
    ...
    def doNotCache(fileName):
        ...
        @param fileName: The file name to check against the XnatUtils.doNotCache list.
    ...
    def isDecompressible(fileName):
        ...
        @param fileName: The file name to check against the GLOB_DECOMPRESSIBLE_EXTENSIONS list.
        @type fileName: string
    ...
    def getSaveTuple( fileUri = None):
        ...
        slicerSaveUri = None
        if fileUri:
            lSplit, rSplit = fileUri.split(GLOB_DEFAULT_XNAT_SAVE_LEVEL + "/")
            saveLevelUri = (lSplit + GLOB_DEFAULT_XNAT_SAVE_LEVEL + "/" + rSplit.split("/")[0])
            slicerSaveUri = saveLevelUri + "/resources/" + GLOB_SLICER_FOLDER_NAME + "/files"
    ...
    def generateButton(iconOrLabel="", toolTip="", font = qt.QFont('Arial', 10, 10, False),  size = None, enabled=False):
        ...
        @param iconOrLabel: Either the icon uri or the label of the button.   Defaults to ''.
